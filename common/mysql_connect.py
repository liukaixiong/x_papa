...
class MysqldbHelper(object):  # 继承object类所有方法
    ...
    def createDataBase(self, DB_NAME):
        ...
        logging.debug('creatDatabase:' + DB_NAME)
    ...
    def update(self, tablename, attrs_dict, cond_dict):
        ...
        logging.debug("attrs_sql:", attrs_sql)
        if cond_dict != '':
            for k, v in cond_dict.items():
                if isinstance(v, str):
                    v = "\'" + v + "\'"
...
if __name__ == "__main__":
    ...
    params = {}
    for i in range(5):
        params.update({"user_id": "1111" + str(i), "username": "aaaaa"})  # 生成字典数据，循环插入
        print(params)
